...
public class WeakOpponent : Character
{
    ...
    {
        baseStats.maxHp = 50 + (info.level - 1) * 5;
        baseStats.maxArmor = 50;
        baseStats.attack = 5 + (info.level - 1) * 2;
        baseStats.speed = 15;
        baseStats.hpRegen = .25f;
    ...
    protected override void DoAttack2()
    {
        ...
        passingAttack.knockback = chipAwayKnockback;
        
        LoadAttack(passingAttack);

        if (passingAttack.target != -1)
        {
            ((ConstantMovement)mover).MoveToAndFollowPlayer(GetEnemyByIndex(passingAttack.target).transform);
        }
    }
    ...
    private class ChippedAway : Buff
    {
        ...
        public ChippedAway(AnimatorOverrideController a, float onHitDamage)
        {
            ...
            posNeg = -1;
            text = "Chipped Armor\nThis unit's armor is becoming increasingly comprimised";
        }
    ...
    {
        DealAttack(1, new BuffCarrier(0, this, new Attack4Effects(cowerSymbol)));
        DealAttack(2, new BuffCarrier(0, this, new Attack4Effects(cowerSymbol)));
        DealAttack(3, new BuffCarrier(0, this, new Attack4Effects(cowerSymbol)));
        DealAttack(4, new BuffCarrier(0, this, new Attack4Effects(cowerSymbol)));

        DealAttackToAllies(1, new BuffCarrier(0, this, new Attack4Effects(cowerSymbol)));
        DealAttackToAllies(2, new BuffCarrier(0, this, new Attack4Effects(cowerSymbol)));
        DealAttackToAllies(3, new BuffCarrier(0, this, new Attack4Effects(cowerSymbol)));
        DealAttackToAllies(4, new BuffCarrier(0, this, new Attack4Effects(cowerSymbol)));
    }
